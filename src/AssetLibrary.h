...
enum class Kind { Folder, Script, Prefab, Scene, Input, Image, Model, Shader, Material, Audio, Lightmap, Data, DataSheet, Font, File };
...
inline Kind Type(const std::filesystem::path& p) {
    ...
    if(ext==L".ttf"||ext==L".otf")return Kind::Font;
    if(ext==L".lightmap")return Kind::Lightmap;
    if(ext==L".zdata")return Kind::Data;
    if(ext==L".zsheet")return Kind::DataSheet;
    return Kind::File;
}
...
inline void Icon(HDC dc,Kind kind,int x,int y) {
    const COLORREF colors[]={RGB(217,178,89),RGB(133,199,144),RGB(120,174,236),RGB(182,159,227),RGB(219,155,107),RGB(137,194,209),RGB(122,179,227),RGB(226,140,196),RGB(120,206,196),RGB(167,174,186),RGB(236,222,140),RGB(150,210,170),RGB(150,196,210),RGB(214,196,150),RGB(167,174,186)};
    ...
    if(kind==Kind::Folder){POINT p[]={{x,y+4},{x+6,y+4},{x+8,y+6},{x+17,y+6},{x+17,y+17},{x,y+17},{x,y+4}};Polyline(dc,p,7);}
        ...
        else if(kind==Kind::Lightmap){Ellipse(dc,x+6,y+3,x+12,y+9);MoveToEx(dc,x+9,y+9,nullptr);LineTo(dc,x+9,y+13);for(int i=0;i<4;++i){MoveToEx(dc,x+4+i*4,y+16,nullptr);LineTo(dc,x+2+i*4,y+19);}}
        else if(kind==Kind::Data){Ellipse(dc,x+4,y+3,x+14,y+7);MoveToEx(dc,x+4,y+5,nullptr);LineTo(dc,x+4,y+15);MoveToEx(dc,x+14,y+5,nullptr);LineTo(dc,x+14,y+15);Ellipse(dc,x+4,y+9,x+14,y+13);Ellipse(dc,x+4,y+13,x+14,y+17);}
        else if(kind==Kind::DataSheet){Rectangle(dc,x+2,y+3,x+16,y+18);MoveToEx(dc,x+2,y+8,nullptr);LineTo(dc,x+16,y+8);MoveToEx(dc,x+2,y+13,nullptr);LineTo(dc,x+16,y+13);MoveToEx(dc,x+7,y+3,nullptr);LineTo(dc,x+7,y+18);MoveToEx(dc,x+11,y+3,nullptr);LineTo(dc,x+11,y+18);}
        else if(kind==Kind::Font){MoveToEx(dc,x+4,y+16,nullptr);LineTo(dc,x+8,y+4);LineTo(dc,x+12,y+16);MoveToEx(dc,x+5,y+12,nullptr);LineTo(dc,x+11,y+12);}
