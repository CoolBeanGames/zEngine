...
std::filesystem::path Export(const projects::Project& project,const std::filesystem::path& startupScene,const Settings& settings,const std::filesystem::path& outputParent,const std::filesystem::path& playerDirectory,const std::function<void(unsigned,const std::string&)>& progress) {
    ...
    Require(std::filesystem::is_regular_file(player/L"zPlayer.exe") && std::filesystem::is_regular_file(player/L"shaders"/L"ColorCube.hlsl") && std::filesystem::is_regular_file(player/L"shaders"/L"ShadowDepth.hlsl") && std::filesystem::is_regular_file(player/L"shaders"/L"Sprite.hlsl") && std::filesystem::is_regular_file(player/L"shaders"/L"Decal.hlsl"),"Standalone player is missing. Build the zPlayer target first.");
    ...
    try {
        ...
        for(const auto* shader:{L"ColorCube.hlsl",L"ShadowDepth.hlsl",L"Sprite.hlsl",L"Decal.hlsl"})std::filesystem::copy_file(player/L"shaders"/shader,staging/L"shaders"/shader);
