...
Document Capture(const ObjectStore& objects,const ScriptHost& scripts)
{
    ...
    for (std::size_t i=0;i<objects.Size();++i)
    {
        ...
        for (std::size_t j=0;j<object.BehaviorCount();++j)
        {
            ...
            else if(const auto* audioSource=dynamic_cast<const audio::AudioSource*>(&behavior)){
                b.kind=BehaviorData::Kind::Audio;b.audioClip=audioSource->Clip();b.audioSpatial=audioSource->Spatial();b.audioAutoplay=audioSource->Autoplay();
            ...
            else if(const auto* lightBehavior=dynamic_cast<const Light*>(&behavior)){
                b.kind=BehaviorData::Kind::Light;b.lightType=static_cast<int>(lightBehavior->LightType());b.lightColor=lightBehavior->Color();
                b.lightIntensity=lightBehavior->Intensity();b.lightRange=lightBehavior->Range();b.lightFalloff=lightBehavior->Falloff();
...
std::string Encode(const Document& scene)
{
    ...
    for (const auto& object:scene.objects)
    {
        ...
        for (const auto& b:object.behaviors)
        {
            ...
            else if(b.kind==BehaviorData::Kind::Light) { // scene v13
                out<<"light "<<b.lightType<<' '<<b.lightColor.x<<' '<<b.lightColor.y<<' '<<b.lightColor.z<<' '<<b.lightIntensity
                   <<' '<<b.lightRange<<' '<<b.lightFalloff<<' '<<b.lightSpotInner<<' '<<b.lightSpotOuter<<' '<<(b.lightStatic?1:0)<<'\n';
            }
...
Document Decode(std::string_view text)
{
    ...
    Token(in,"ZENGINE_SCENE"); const auto version=Count(in,13); Require(version>=1,"Unsupported scene version."); Token(in,"objects");
    ...
    for (std::size_t i=0;i<count;++i)
    {
        ...
        for (std::size_t j=0;j<behaviors;++j)
        {
            ...
            if(type=="collider"){Token(in,"collider_shape");b.shape=static_cast<physics::ColliderShape>(Count(in,2));if(version>=5){b.colliderOffset=Vector(in);b.colliderSize=Vector(in);Require(b.colliderSize.x>0&&b.colliderSize.y>0&&b.colliderSize.z>0,"Invalid collider size.");}}
                ...
                Token(in,"audio"); b.audioClip=Text(in);
                if(!b.audioClip.empty()){Asset(b.audioClip,false);Require(audio::IsAudioFile(b.audioClip),"Expected an audio file (.wav/.mp3/.flac/.ogg).");}
                b.audioSpatial=Boolean(in); b.audioAutoplay=Boolean(in); b.audioLoop=Boolean(in);
                ...
                Token(in,"light"); b.lightType=static_cast<int>(Count(in,2));
                b.lightColor=Vector(in); b.lightIntensity=Float(in); b.lightRange=Float(in); b.lightFalloff=Float(in);
                b.lightSpotInner=Float(in); b.lightSpotOuter=Float(in); b.lightStatic=Boolean(in);
                Require(b.lightIntensity>=0 && b.lightRange>0 && b.lightFalloff>0 && b.lightSpotOuter>b.lightSpotInner,"Invalid light settings.");
            }
...
Instance Instantiate(const Document& scene)
{
    ...
    for (const auto& data:scene.objects)
    {
        ...
        for (const auto& b:data.behaviors)
        {
            ...
            else if(b.kind==BehaviorData::Kind::Light){auto& v=object.AddBehavior<Light>();v.SetLightType(static_cast<Light::Type>(b.lightType));v.SetColor(b.lightColor);v.SetIntensity(b.lightIntensity);v.SetRange(b.lightRange);v.SetFalloff(b.lightFalloff);v.SetSpotOuter(b.lightSpotOuter);v.SetSpotInner(b.lightSpotInner);v.SetStatic(b.lightStatic);behavior=&v;}
