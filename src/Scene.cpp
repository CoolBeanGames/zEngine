    ...
    for (std::size_t i=0;i<objects.Size();++i)
    {
        ...
        for (std::size_t j=0;j<object.BehaviorCount();++j)
        {
            ...
            { b.kind=BehaviorData::Kind::Script; b.asset=script->Asset(); b.variables=scripts.AuthoredValues(*script); b.objectReferences=scripts.AuthoredReferences(*script); b.arrays=scripts.AuthoredArrays(*script); }
                ...
                b.envHeightBase=environment->HeightBase();b.envHeightFalloff=environment->HeightFalloff();b.envHeightStrength=environment->HeightStrength();
                b.envVolumetric=environment->Volumetric();b.envVolumetricSteps=environment->VolumetricSteps();
            }
...
std::string Encode(const Document& scene)
{
    ...
    for (const auto& object:scene.objects)
    {
        ...
        for (const auto& b:object.behaviors)
        {
            ...
            else if(b.kind==BehaviorData::Kind::Light) { // scene v13 (+ fogScatter v14)
            ...
            else if(b.kind==BehaviorData::Kind::Environment) { // scene v14
                out<<"environment "<<b.envFogMode<<' '<<b.envFogColor.x<<' '<<b.envFogColor.y<<' '<<b.envFogColor.z
    ...
    for (std::size_t i=0;i<count;++i)
    {
        ...
        for (std::size_t j=0;j<behaviors;++j)
        {
            ...
            if(type=="collider"){Token(in,"collider_shape");b.shape=static_cast<physics::ColliderShape>(Count(in,2));if(version>=5){b.colliderOffset=Vector(in);b.colliderSize=Vector(in);Require(b.colliderSize.x>0&&b.colliderSize.y>0&&b.colliderSize.z>0,"Invalid collider size.");}}
                ...
                if(version>=14)b.lightFogScatter=Float(in);
                Require(b.lightIntensity>=0 && b.lightRange>0 && b.lightFalloff>0 && b.lightSpotOuter>b.lightSpotInner && b.lightFogScatter>=0,"Invalid light settings.");
            }
            else if(type=="environment"){
                Token(in,"environment"); b.envFogMode=static_cast<int>(Count(in,2));
                b.envFogColor=Vector(in); b.envFogNear=Float(in); b.envFogFar=Float(in); b.envFogDensity=Float(in);
