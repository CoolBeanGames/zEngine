...
GameObjects, attachments, priorities, mesh assignments, and authored Inspector variables persist when you save the current **scene asset**. Project creation/selection UI, parenting, scene undo/redo, and viewport picking/manipulator dragging are still deferred.

## Scene assets

- Click **+ New Scene** in the Media Library, use **File > New Scene**, or right-click the library and choose **Create Scene (.zscene)**. This creates a uniquely named `NewScene.zscene` (numbered as needed) inside the current project's `Assets` directory and opens it with an empty Scene Browser. The original startup cube belongs to an untitled scene; use Save Scene to keep it.
...
The current tree remains flat; parent/child relationships and cross-object script reference serialization will come with their corresponding systems. Scene files can be reopened after restarting the editor; automatic reopening of the last scene and a project-management UI are not implemented yet.
