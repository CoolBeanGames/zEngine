...
Distribute that entire folder: **`YourGame.exe`**, **`Data/`**, **`shaders/`**, and the included third-party license. The player loads files relative to its executable, not the working directory. It has its own window and uses the same scene, prefab, script lifecycle, input, FBX/albedo, and rendering systems as the editor. It does not start or require the editor. The C++ runtime is statically linked; Visual Studio and an MSVC redistributable installation are not needed on the target PC. Target: Windows 10/11 x64 with Direct3D 11.

All registered scenes and the project's Assets contents are packaged, preserving folder paths and prefab dependencies. Scripts compile through the embedded VM at startup; no external compiler is needed, but source is included (not encrypted). Missing scenes/models/scripts, broken prefab graphs, invalid Input Maps, and compile errors fail validation. Symlinks/junctions in Assets are rejected. Source assets and older builds are never replaced. Export currently packages up to 100,000 files / 8 GiB; asset stripping, compression, an installer, and non-Windows targets are not implemented.
...
An empty object contains only its transform and metadata: it has no mesh and no behaviors by default. The selected object's **editor-only transform handles** use X red, Y green, and Z blue against a world grid. These guides are not runtime GameObject components. A cube is a regular GameObject with a Mesh Renderer behavior.
...
New saves use `ZENGINE_SCENE 3`, tracking prefab transform overrides per XYZ component; versions 1 and 2 still load. Moving an instance no longer overrides its inherited scale or rotation. Legacy all-transform overrides are migrated by comparing against the current prefab source (historical source values cannot be recovered). The tree indents children. Cross-object script reference serialization remains deferred. The last scene is reopened automatically through its project config on editor startup.
...
- Scripts can expose a project prefab with `export prefab template;`. Click its Inspector button to choose a `.zprefab`, or drag a prefab asset directly onto that field. `gameObject made = template.spawn();` appends the resolved prefab hierarchy to the live scene and returns its root, so the script can immediately move, rotate, parent, or otherwise control it. Runtime-spawned objects are discarded when Play stops and work in standalone builds.
...
- Right-click **Refresh Assets** to discover files added externally; double-click folders to browse their contents. Dropped `.zsh` files and images are copied into the current folder without overwriting existing files. Script renaming and behavior removal are not implemented yet.
...
- `export` exposes `int`, `float`, `bool`, `string`, and `Vector3` fields. Enter booleans as `true`/`false`, vectors in separate X/Y/Z boxes, and strings without surrounding quotes. `label("text")` inserts text in declaration order, with inherited entries first; its semicolon is optional. Other fields remain hidden. Exported class/array references are shown read-only.
