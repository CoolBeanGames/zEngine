...
int main(){
    ...
    const auto x=ball.GetTransform().Position().x;const auto hits=world.Cast({x,5,0},{x,-5,0});assert(!hits.empty());assert(hits.front().object==ball.Id());bool received=false,fixed=false;for(const auto& field:host.Fields(script)){if(field.name=="contacts")received=std::stoll(field.value)>0;if(field.name=="physics_ticks")fixed=std::stoll(field.value)==240;}assert(received&&fixed);assert(ball.GetBehavior<MeshRenderer>() && As3D(ball.GetBehavior<MeshRenderer>()->Owner()).GetTransform().Position()==ball.GetTransform().Position());host.Stop(objects);
