...
int main(){
    ...
    ScriptHost host;const auto source=R"(class PhysicsMover : gameObject { export int contacts=0; export int physics_ticks=0; func start() { physics.collision_entered.connect(on_collision); physics.add_impulse(transform.right*2); } func physicsUpdate(float delta) { physics_ticks+=1; } func on_collision(gameObject other) { contacts+=1; } })";assert(host.Prepare(script,source,"PhysicsMover"));
